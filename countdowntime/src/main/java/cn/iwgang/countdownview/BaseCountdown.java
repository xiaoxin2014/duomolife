...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, 28);
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, 28);
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    protected void initPaint() {
        ...
        mTimeTextPaint.setTextSize(Utils.autoPx(mContext, mTimeTextSize));
        if (isTimeTextBold) {
        ...
        mSuffixTextPaint.setTextSize(Utils.autoPx(mContext, mSuffixTextSize));
        if (isSuffixTextBold) {
            mSuffixTextPaint.setFakeBoldText(true);
        }

        mMeasureHourWidthPaint = new Paint();
        mMeasureHourWidthPaint.setTextSize(Utils.autoPx(mContext, mTimeTextSize));
        if (isTimeTextBold) {
...
//            更改
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
//            更改
            mSuffixTextPaint.setTextSize(textSize);
        }
    }
