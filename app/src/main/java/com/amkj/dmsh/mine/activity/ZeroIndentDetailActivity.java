...
public class ZeroIndentDetailActivity extends BaseActivity {
    ...
    private void setDetailInfo() {
        ...
        if (!TextUtils.isEmpty(second)) {
            ...
            //主订单按钮
            mMainButtonView.setVisibility(View.VISIBLE);
            MainOrderBean mainOrderBean = new MainOrderBean();
            mainOrderBean.setOrderNo(mZeroIndentBean.getOrderNo());
            ButtonListBean buttonListBean = new ButtonListBean(String.valueOf(GO_PAY), "1", "去支付");
            List<ButtonListBean> buttonList = new ArrayList<>();
            buttonList.add(buttonListBean);
            mainOrderBean.setButtonList(buttonList);
            mMainButtonView.updateView(this, mainOrderBean, getSimpleName());
        } else {
            mTvCountdownTime.setVisibility(GONE);
            mMainButtonView.setVisibility(GONE);
        }
    ...
    @OnClick({R.id.tv_life_back, R.id.ll_qy_service, R.id.tv_copy_text})
