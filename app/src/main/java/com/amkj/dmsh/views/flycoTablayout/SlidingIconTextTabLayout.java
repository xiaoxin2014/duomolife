 ...
 */
public class SlidingIconTextTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    private Context mContext;
    private ViewPager mViewPager;
    private ArrayList<PowerEntity.PowerBean> HomeNavbarList;
     ...
     */
    public void setViewPager(ViewPager vp, List<PowerEntity.PowerBean> list) {
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.layout_tab_icon_text, null);
            CharSequence pageTitle = HomeNavbarList.size() == 0 ? mViewPager.getAdapter().getPageTitle(i) : HomeNavbarList.get(i).getTitle();
    ...
    private void updateTabStyles() {
        try {
            for (int i = 0; i < mTabCount; i++) {
                ...
                PowerEntity.PowerBean powerInfoBean = HomeNavbarList.get(i);
                ...
                if (tv_tab_title != null && iv_tab_title != null) {
                    ...
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, AutoSizeUtils.mm2px(mAppContext, isSelect ? 26 : 22));
                    ...
                    if (isContextExisted(mContext)) {
                        //给图片重新设置一个合适的尺寸
                        Glide.with(mContext).load(powerInfoBean.getIcon())
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; i++) {
            ...
            ImageView iv_tab_title = tabView.findViewById(R.id.iv_tab_title);
            if (tv_tab_title != null && iv_tab_title != null) {
                tv_tab_title.setTypeface(Typeface.defaultFromStyle(isSelect ? Typeface.BOLD : Typeface.NORMAL));//加粗
                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, AutoSizeUtils.mm2px(mAppContext, isSelect ? 26 : 22));
                if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
