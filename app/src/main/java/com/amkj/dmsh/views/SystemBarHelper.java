...
public class SystemBarHelper {
    ...
    private static int DEFAULT_COLOR = 0xf7f6f1;
    ...
    public static void tintStatusBar(Activity activity) {
        tintStatusBar(activity, DEFAULT_COLOR, DEFAULT_ALPHA);
    }
    ...
    public static void tintStatusBar(Activity activity, @ColorInt int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            float alphas = (255 * alpha);
            window.setStatusBarColor(Color.argb((int) alphas, Color.red(statusBarColor), Color.green(statusBarColor), Color.blue(statusBarColor)));
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (statusBarColor == 0) {
                statusBarColor = activity.getResources().getColor(R.color.blackColor);
            }
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            float alphas = (255 * alpha);
    ...
    public static void tintStatusBarForDrawer(Activity activity, DrawerLayout drawerLayout, @ColorInt int statusBarColor,
        ...
        setTranslucentView(decorView, statusBarColor, alpha);
    ...
    public static void immersiveStatusBar(Activity activity) {
        DEFAULT_COLOR = activity.getResources().getColor(R.color.gray_bg);
        immersiveStatusBar(activity, DEFAULT_ALPHA, DEFAULT_COLOR);
    }
     ...
     *
     * @param window         一般都是用于Activity的window,也可以是其他的例如Dialog,DialogFragment
     * @param statusBarColor 十六进制的颜色值
     */
    public static void immersiveStatusBar(Activity activity, int statusBarColor) {
        immersiveStatusBar(activity, DEFAULT_ALPHA, statusBarColor);
    }
    ...
    public static void immersiveStatusBar(Activity activity, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        immersiveStatusBar(activity, alpha, DEFAULT_COLOR);
    }
     ...
     */
    public static void immersiveStatusBar(Activity activity, @FloatRange(from = 0.0, to = 1.0) float alpha, int statusBarColor) {
        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            boolean isFontsBlack = setStatusBarDarkMode(activity);
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            float alphas = (255 * alpha);
            if (!isFontsBlack) {
            ...
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            float alphas = (255 * alpha);
            if (statusBarColor != 0 && statusBarColor != 0xffffff) {
                window.setStatusBarColor(Color.argb((int) alphas, Color.red(statusBarColor), Color.green(statusBarColor), Color.blue(statusBarColor)));
            } else {
                window.setStatusBarColor(Color.argb((int) alphas, Color.red(statusBarColor), Color.green(statusBarColor), Color.blue(statusBarColor)));
            }
...
//    }
     ...
     * 创建假的透明栏
     */
    public static void setTranslucentView(ViewGroup container, int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (alpha == 0 && statusBarColor == 0) {
                translucentView.setBackgroundColor(0x7fffffff);
            } else if (alpha == 0) {
                translucentView.setBackgroundColor(statusBarColor);
            } else {
                float alphas = 255 * alpha;
                translucentView.setBackgroundColor(Color.argb((int) alphas,
                        Color.red(statusBarColor), Color.green(statusBarColor), Color.blue(statusBarColor)));
            }
        }
    }
