package com.amkj.dmsh.views.xcpulltoloadmorelistview;
...
public class XCPullToLoadMoreListView extends FrameLayout {
    ...
    public static final int REFRESH_STATUS_NO_MORE = 4;//"已无更多数据";
        ...
        backToMiddleAnim.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mOnRefreshListener != null) {
                    mLastCount = mListView.getLayoutManager().getItemCount();
    ...
    private void completeBackToTop() {
        ...
        int num = mListView.getAdapter().getItemCount() - (mLastCount + 1);
        Log.v("czm", "num=" + num);
        if (num > 0) {
            (mListView.getLayoutManager()).scrollToPosition(num);
        }
    }

    public void onRefreshComplete(boolean noMore) {
        mLastTranslationY = mListView.getTranslationY();
        mIsRefreshing = false;
        mRefreshStatus = noMore ? REFRESH_STATUS_NO_MORE : REFRESH_STATUS_REFRESH_FINISH;
        mRefreshHeader.updateRefreshStatus(mRefreshStatus);
        completeBackToTop();
    }
