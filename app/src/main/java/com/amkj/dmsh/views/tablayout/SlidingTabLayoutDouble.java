 ...
 */
public class SlidingTabLayoutDouble extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    private Context mContext;
    private ViewPager mViewPager;
    private ArrayList<CustomTabDoubleEntity> mTitles;
    ...
    public SlidingTabLayoutDouble(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingTabLayoutDouble(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SlidingTabLayoutDouble);

        mIndicatorStyle = ta.getInt(R.styleable.SlidingTabLayoutDouble_tl_indicator_style, STYLE_NORMAL);
        mIndicatorColor = ta.getColor(R.styleable.SlidingTabLayoutDouble_tl_indicator_color, Color.parseColor(mIndicatorStyle == STYLE_BLOCK ? "#4B6A87" : "#ffffff"));
        mIndicatorHeight = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_height,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 4 : (mIndicatorStyle == STYLE_BLOCK ? -1 : 2)));
        mIndicatorWidth = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_width, dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 10 : -1));
        mIndicatorCornerRadius = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_corner_radius, dp2px(mIndicatorStyle == STYLE_BLOCK ? -1 : 0));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_margin_left, dp2px(0));
        mIndicatorMarginTop = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_margin_top, dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 0));
        mIndicatorMarginRight = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_margin_right, dp2px(0));
        mIndicatorMarginBottom = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_indicator_margin_bottom, dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 0));
        mIndicatorGravity = ta.getInt(R.styleable.SlidingTabLayoutDouble_tl_indicator_gravity, Gravity.BOTTOM);
        mIndicatorWidthEqualTitle = ta.getBoolean(R.styleable.SlidingTabLayoutDouble_tl_indicator_width_equal_title, false);

        mUnderlineColor = ta.getColor(R.styleable.SlidingTabLayoutDouble_tl_underline_color, Color.parseColor("#ffffff"));
        mUnderlineHeight = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_underline_height, dp2px(0));
        mUnderlineGravity = ta.getInt(R.styleable.SlidingTabLayoutDouble_tl_underline_gravity, Gravity.BOTTOM);

        mDividerColor = ta.getColor(R.styleable.SlidingTabLayoutDouble_tl_divider_color, Color.parseColor("#ffffff"));
        mDividerWidth = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_divider_width, dp2px(0));
        ...
        mTabWidth = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_tab_width, dp2px(-1));
        mTabPadding = ta.getDimension(R.styleable.SlidingTabLayoutDouble_tl_tab_padding, mTabSpaceEqual || mTabWidth > 0 ? dp2px(0) : dp2px(20));
...
//    }
     ...
     * 关联ViewPager,用于不想在ViewPager适配器中设置titles数据的情况
     */
    public void setViewPager(ViewPager vp, List<CustomTabDoubleEntity> titles) {
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }
    ...
    public void notifyDataSetChanged() {
        ...
        if (mTitles != null && mTitles.size() > 0) {
            ...
            for (int i = 0; i < mTabCount; i++) {
                tabView = View.inflate(mContext, R.layout.layout_tab_double_center, null);
    ...
    public void addNewTab(CustomTabDoubleEntity customTabDoubleEntity) {
        if (customTabDoubleEntity == null) {
            throw new IllegalStateException("Object can not be EMPTY !");
        }
        View tabView = View.inflate(mContext, R.layout.layout_tab_double_center, null);
        if (mTitles != null && customTabDoubleEntity != null) {
            mTitles.add(customTabDoubleEntity);
        }
        addTab(mTabCount, customTabDoubleEntity, tabView);
        this.mTabCount = mTitles.size();
        updateTabStyles();
    }
     ...
     * 创建并添加tab
     */
    private void addTab(final int position, CustomTabDoubleEntity tabDoubleEntity, View tabView) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        TextView tv_tab_subtitle = (TextView) tabView.findViewById(R.id.tv_tab_subtitle);
        if (tv_tab_title != null) {
            if (tabDoubleEntity != null){
                tv_tab_title.setText(tabDoubleEntity.getTabTitle());
            }
        }

        if (tv_tab_subtitle != null) {
            if (tabDoubleEntity != null && !TextUtils.isEmpty(tabDoubleEntity.getTabSubtitle())){
                tv_tab_subtitle.setText(tabDoubleEntity.getTabSubtitle());
            }else{
                tv_tab_subtitle.setText("");
            }
        }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tv_tab_subtitle != null) {
                tv_tab_subtitle.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
                if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
                    tv_tab_subtitle.getPaint().setFakeBoldText(isSelect);
                }
            }
        }
    }
