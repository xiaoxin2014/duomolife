...
public class TextParser {
     ...
     * 添加带链接的文字
     */
    public TextParser append(String text, int size, int color, dataClick onClickListener) {
        if (text == null) {
            return this;
        }
        TextBean bean = new TextBean();
    ...
    public void parse(TextView textView) {
        ...
        for (TextBean bean : textList) {
            if (bean.onClickListener != null) {
                ...
                style.setSpan(new MyClickableSpan(bean.onClickListener, bean.text),// Span接口用于实现对文本的修饰的具体内容
                        position,// 修饰的起始位置
    ...
    private class TextBean {
        public dataClick onClickListener;
        public String text;
        public int size;
        public int color;
    }
    ...
    private static class MyClickableSpan extends ClickableSpan {
        ...
        private dataClick mOnClickListener;

        public MyClickableSpan(dataClick onClickListener, String text) {
    ...
    public void OnClickListener(dataClick onClick) {
        this.mOnClickListener = onClick;
    }

    public interface dataClick {
        void date(String text);
    }
 /*
  * 使用这个类改变文本样式共分三步：
  创建TextParser类
