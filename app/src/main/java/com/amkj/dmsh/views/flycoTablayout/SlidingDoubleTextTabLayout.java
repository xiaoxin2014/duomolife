 ...
 */
public class SlidingDoubleTextTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    private Context mContext;
    private ViewPager mViewPager;
    private ArrayList<TimeAxisEntity.TimeAxisBean> HomeNavbarList;
    ...
    public SlidingDoubleTextTabLayout(Context context) {
        this(context, null, 0);
    }

    public SlidingDoubleTextTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingDoubleTextTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    public void setViewPager(ViewPager vp, List<TimeAxisEntity.TimeAxisBean> list) {
        mCurrentTab = 0;
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.layout_tab_double_text, null);
            CharSequence title = HomeNavbarList.get(i).getShowDate();
            CharSequence subtitle = HomeNavbarList.get(i).getName();
            addTab(i, title.toString(), subtitle.toString(), tabView);
        }
     ...
     * 创建并添加tab
     */
    private void addTab(final int position, String title, String subTitle, View tabView) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        TextView tv_tab_subtitle = (TextView) tabView.findViewById(R.id.tv_tab_subtitle);
        if (tv_tab_title != null) {
            if (title != null) tv_tab_title.setText(title);
            tv_tab_title.setSelected(position == mCurrentTab);
        }

        if (tv_tab_subtitle != null) {
            if (subTitle != null) tv_tab_subtitle.setText(subTitle);
            tv_tab_subtitle.setSelected(position == mCurrentTab);
        }
    ...
    private void updateTabStyles() {
        try {
            for (int i = 0; i < mTabCount; i++) {
                ...
                TextView tv_tab_subtitle = v.findViewById(R.id.tv_tab_subtitle);
                if (tv_tab_title != null && tv_tab_subtitle != null) {
                    tv_tab_title.setSelected(isSelect);
                    tv_tab_subtitle.setSelected(isSelect);
                    lLContent.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
                    if (mTextAllCaps) {
