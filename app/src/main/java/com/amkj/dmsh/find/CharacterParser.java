...
/**
 * Java汉字转换为拼音，此工具类有bug,获取拼音时，“斐”类似繁体字不能识别到；
 */
public class CharacterParser {
    ...
    public static String getSpells(String characters) {
        ...
        for (int i = 0; i < characters.length(); i++) {
            ...
            String s = String.valueOf(ch).toString();
            s.length();
            Log.d("spells", "getSpells: " + s + "\t" + s.length() + "\t" + (ch >> 7));
            if ((ch >> 7) == 0 || String.valueOf(ch).indexOf("\\") == 0) {
                // 判断是否为汉字，如果左移7为0就不是汉字，否则是汉字
            } else {
                if (getFirstLetter(ch) != null) {
