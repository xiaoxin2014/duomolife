...
public class CameraComponentSample extends SampleBase implements TuCameraFragmentDelegate {
    ...
    public static final String DEFAULT_PATH = "cache";
    ...
    public void showSample(Activity activity, String type, int positionImg) {
        ...
        // 是否开启音量键拍照功能，默认关闭
        option.setEnableCaptureWithVolumeKeys(false);
...
//	    option.setMarkImage(BitmapHelper.getRawBitmap(activity, R.raw.sample_watermark));
    ...
    public void onTuCameraFragmentCaptured(TuCameraFragment fragment, TuSdkResult result) {
        ...
        if (type != null) {
            replaceData.setPath(result.imageSqlInfo.path);
            replaceData.setPosition(positionImg);
            EventBus.getDefault().post(new EventMessage(type, replaceData));
        }
