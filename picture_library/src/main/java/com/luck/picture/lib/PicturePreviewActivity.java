...
import com.luck.picture.lib.adapter.SimpleFragmentAdapterC;
...
public class PicturePreviewActivity extends PictureBaseActivity implements
        View.OnClickListener, Animation.AnimationListener, SimpleFragmentAdapterC.OnCallBackActivity {
    ...
    private List<LocalMediaC> selectImages = new ArrayList<>();
    private TextView check;
    private SimpleFragmentAdapterC adapter;
    ...
    public void eventBus(EventEntity obj) {
        switch (obj.what) {
            case PictureConfigC.CLOSE_PREVIEW_FLAG:
                // 压缩完后关闭预览界面
                dismissDialog();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        position = getIntent().getIntExtra(PictureConfigC.EXTRA_POSITION, 0);
        ...
        selectImages = (List<LocalMediaC>) getIntent().
                getSerializableExtra(PictureConfigC.EXTRA_SELECT_LIST);
        boolean is_bottom_preview = getIntent().
                getBooleanExtra(PictureConfigC.EXTRA_BOTTOM_PREVIEW, false);
        if (is_bottom_preview) {
            // 底部预览按钮过来
            images = (List<LocalMediaC>) getIntent().
                    getSerializableExtra(PictureConfigC.EXTRA_PREVIEW_SELECT_LIST);
        } else {
        ...
        ll_check.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (images != null && images.size() > 0) {
                    LocalMediaC image = images.get(viewPager.getCurrentItem());
                    ...
                    if (isChecked) {
                        ...
                        for (LocalMediaC media : selectImages) {
    ...
    private void singleRadioMediaImage() {
        if (selectImages != null
                && selectImages.size() > 0) {
            LocalMediaC media = selectImages.get(0);
            RxBus.getDefault()
                    .post(new EventEntity(PictureConfigC.UPDATE_FLAG,
    ...
    private void initViewPageAdapterData() {
        tv_title.setText(position + 1 + "/" + images.size());
        adapter = new SimpleFragmentAdapterC(images, this, this);
     ...
     * 选择按钮更新
     */
    private void notifyCheckChanged(LocalMediaC imageBean) {
        if (config.checkNumMode) {
            check.setText("");
            for (LocalMediaC media : selectImages) {
    ...
    private void subSelectPosition() {
        for (int index = 0, len = selectImages.size(); index < len; index++) {
            LocalMediaC media = selectImages.get(index);
     ...
     * @return
     */
    public boolean isSelected(LocalMediaC image) {
        for (LocalMediaC media : selectImages) {
            if (media.getPath().equals(image.getPath())) {
                return true;
            }
    ...
    private void updateSelector(boolean isRefresh) {
        if (isRefresh) {
            EventEntity obj = new EventEntity(PictureConfigC.UPDATE_FLAG, selectImages, index);
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.id_ll_ok) {
            ...
            LocalMediaC image = selectImages.size() > 0 ? selectImages.get(0) : null;
            String pictureType = image != null ? image.getPictureType() : "";
            if (config.minSelectNum > 0) {
                if (size < config.minSelectNum && config.selectionMode == PictureConfigC.MULTIPLE) {
            ...
            if (config.enableCrop && pictureType.startsWith(PictureConfigC.IMAGE)) {
                if (config.selectionMode == PictureConfigC.SINGLE) {
    ...
    @Override
    public void onResult(List<LocalMediaC> images) {
        RxBus.getDefault().post(new EventEntity(PictureConfigC.PREVIEW_DATA_FLAG, images));
